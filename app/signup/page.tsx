...
export const metadata: Metadata = {
  title: "Create account — Nepal",
  description: "Join the Nepal community — plan your trek, share your stories, book adventures.",
};
...
export default function SignupPage() {
  return (
    <div className="relative min-h-screen flex flex-col bg-[#07070d]">
      ...
      {/* ── Background photo — Swayambhunath (the all-seeing eyes + prayer flags) ── */}
      <Image
        src="/Swayambhunath.jpg"
        alt=""
        fill
        priority
        sizes="100vw"
        style={{ objectFit: "cover", objectPosition: "center 30%", filter: "brightness(.8) saturate(.72)", zIndex: 0 }}
      />

      {/* Dark scrim — heavier on right where form sits */}
      <div
        aria-hidden="true"
        style={{
          position: "absolute", inset: 0, zIndex: 1,
          background: "linear-gradient(110deg, rgba(7,7,13,.35) 0%, rgba(7,7,13,.52) 50%, rgba(7,7,13,.8) 100%)",
        }}
      />
      ...
      {/* Crimson ambient blob — bottom-left */}
      <div
        aria-hidden="true"
        style={{
          ...
          background: "radial-gradient(circle, rgba(156,28,58,.15) 0%, transparent 65%)",
          filter: "blur(72px)",
        }}
      />
      ...
      >
        <div className="max-w-[1200px] mx-auto px-6 md:px-10 w-full grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          <div
            className="glass-dark rounded-3xl p-8 md:p-10 lg:order-1 order-1"
          ...
          >
            ...
            >
              Already a member?{" "}
              <Link href="/login" className="text-[#d4a843] hover:underline font-semibold">
                Sign in →
              </Link>
            </p>
            ...
            <form className="flex flex-col gap-5">
              ...
              <label className="flex items-center gap-3 cursor-pointer select-none">
                ...
                >
                  Send me travel stories &amp; seasonal guides
                </span>
              </label>
          ...
          <div className="flex flex-col lg:order-2 order-2">
            ...
            >
              Join the community · समुदायमा सामेल हुनुहोस्
            </p>
            <h1
            ...
            >
              नेपाल यात्रा सुरु गर्नुहोस्
            </p>
            <p
              className="text-[14px] leading-relaxed mb-8 max-w-sm"
              style={{ fontFamily: "var(--font-syne)", color: "rgba(240,236,227,.62)" }}
            >
              Plan your trek, book adventures, and share stories with 18,400 travellers from 94 countries.
            </p>

            {/* Feature list */}
            <div className="flex flex-col gap-4 mb-8">
              {[
                { icon: "🗓️", text: "Instant booking confirmation with .ics calendar" },
                { icon: "🏔️", text: "Curated treks, basecamps and cultural routes" },
                { icon: "✍️", text: "Share your stories with the global Nepal community" },
              ].map((f) => (
                <div key={f.text} className="flex items-start gap-4">
                  <span className="text-xl mt-0.5 shrink-0" aria-hidden="true">{f.icon}</span>
                  <p
                    className="text-[13px] leading-relaxed"
                    style={{ fontFamily: "var(--font-syne)", color: "rgba(240,236,227,.65)" }}
                  >
                    {f.text}
                  </p>
                </div>
            ...
            <PrayerFlagDivider style={{ maxWidth: 140 }} opacity={0.6} />
          </div>
